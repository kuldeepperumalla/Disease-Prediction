...
def load_data(name, plotdir, print_out=True):
    ...
    df = read_data(name)
    train, test = train_test_split(df, test_size=0.30)#test size
...
def fit_predict(clf, train_X, train_y, test_X, test_y, label='x'):
    ...
    test=np.array([32,1,2,134,82,150,1,62,0.1,0,2,8,1,1,2,1]).reshape(1,-1)
    print("prediction value is :", clf.predict(test))
